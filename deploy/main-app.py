...
# import torch
# # Load DPR models
# query_encoder = DPRQuestionEncoder.from_pretrained('facebook/dpr-question_encoder-single-nq-base').to('cuda')
...
def reranker(query, contexts,  images_to_plot_, k=3):
    ...
    # Compute embeddings
    with torch.no_grad():
        # query_emb = query_encoder(**query_input).pooler_output  # Use pooler_output for DPR
        # ctx_emb = context_encoder(**ctx_input).pooler_output
        
        query_emb = query_encoder(**query_input).last_hidden_state[:, 0, :]
        ctx_emb = context_encoder(**ctx_input).last_hidden_state[:, 0, :]
        
    # Compute similarity scores using dot product
    scores = {i: (query_emb @ ctx_emb[i].T).item() for i in range(len(ctx_emb))}
    print(scores)
    print('DPR Done')
    ...
    print(sorted_scores)
    top = [(list(sorted_scores)[i][0], list(sorted_scores)[i][1]) for i in range(min(43, len(sorted_scores))) if list(sorted_scores)[i][1] >= RELEVENCE_THREASHOLD_Dpr]#if list(sorted_scores)[i][1] >= RELEVENCE_THREASHOLD_Dragon
...
def process_documents_Wimages(collection, query_text, n_results=3, max_tables=50):
...
def generate_answer(query,n_results=10,rerank_k=3,  model=model_agent, tokenizer=tokenizer_agent):
    ...
    formatted_docs_retreived, chunks_list, images = process_documents_Wimages(collection, query_text=query, n_results=n_results)
    if chunks_list ==[]:
        formatted_docs_retreived = 'No Relevant Context'
    else:
        formatted_docs, selected_docs, images_to_plot = reranker(query, contexts=chunks_list, images_to_plot_=images, k=rerank_k)#3
...
def generate_pipeline(query, main_model=model_agent, main_tokenizer=tokenizer_agent):
    ...
    if "APPROVED QUERY" in first_layer_response:
        return generate_answer(query,n_results=10, rerank_k=3, model=main_model, tokenizer=main_tokenizer)
    else:
